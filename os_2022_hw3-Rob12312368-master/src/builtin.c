...
int add(char **args)
{
	...
	strcat(tasks[size].resources,"none");
...
int ps(char **args)
{
	char* arr[4] = {"WAITING","TERMINATED","READY","RUNNING"};
	printf("%6s%11s%11s%11s%11s%10s%10s","TID|","name|","state|","running|","waiting|","turnaround|","resources");
	if(!strcmp(algor,"PP"))
		printf("%10s\n","priority");
	else
		printf("\n");
	...
	{
		if(tasks[i].state != TERMI)
			printf("%5d|%10s|%10s|%10d|%10d|%10s|%10s",i,tasks[i].name,arr[tasks[i].state+1],tasks[i].running,tasks[i].waiting,"none",tasks[i].resources);
		else
			printf("%5d|%10s|%10s|%10d|%10d|%10d|%10s",i,tasks[i].name,arr[tasks[i].state+1],tasks[i].running,tasks[i].waiting,tasks[i].turnaround,tasks[i].resources);
		if(!strcmp(algor,"PP"))
			printf("%10d\n",tasks[i].prior);
		else
			printf("\n");
